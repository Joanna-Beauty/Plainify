...
const fakeProvider = http.createServer(async (request, response) => {
  ...
  const modelsByCredential = new Map([
    ['Bearer sk-initial-deepseek-test', ['deepseek-chat', 'deepseek-reasoner']],
    ['Bearer sk-isolated-openai-test', ['text-embedding-3-small', 'gpt-4o-mini', 'gpt-4.1-mini', 'gpt-5.2']],
    ['Bearer sk-replacement-openai-next', ['text-embedding-3-small', 'gpt-4o-mini', 'gpt-4.1-mini', 'gpt-5.2']],
  ])
  const availableModels = modelsByCredential.get(request.headers.authorization)
  ...
  if (request.url === '/v1/models') {
    response.end(JSON.stringify({
      data: availableModels.map((id) => ({ id })),
    }))
    return
  }
  if (request.url === '/v1/chat/completions') {
    ...
    const content = body.max_tokens === 450
      ? JSON.stringify({ explanation: '当前模型生成的解释', analogy: '像按当前线路发车。' })
      : '连接成功'
...
process.env.DEEPSEEK_API_KEY = 'sk-initial-deepseek-test'
...
assert.deepEqual(modelProviders.map((provider) => provider.id), [
  'deepseek',
  'openai',
  'qwen',
  'moonshot',
...
const normalizedCatalogs = normalizeProviderSettings({
  ...
  providerModels: { deepseek: 'deepseek-reasoner', openai: 'gpt-5.2' },
})
...
assert.equal(normalizedCatalogs.providerModels.deepseek, 'deepseek-reasoner')
assert.equal(normalizedCatalogs.providerModels.openai, 'gpt-5.2')
assert.equal(normalizedCatalogs.providerModels.qwen, getProvider('qwen').defaultModel)
...
try {
  ...
  assert.equal(initial.providers.find((provider) => provider.id === 'deepseek').configured, true)
  ...
  assert.equal(saved.savedModel, 'gpt-4o-mini')
  assert.equal(saved.activeProvider, 'deepseek')
  assert.equal(saved.activeModel, 'deepseek-reasoner')
  ...
  assert.equal(completionModels.length, 0)

  const overviewAfterSecondProvider = await (await fetch(`${baseUrl}/ai/providers`, {
    headers: websiteHeaders,
  })).json()
  assert.equal(overviewAfterSecondProvider.activeProvider, 'deepseek')
  assert.equal(overviewAfterSecondProvider.providers.filter((provider) => provider.configured).length, 2)
  ...
  assert.equal(replaced.savedModel, 'gpt-4.1-mini')
  assert.equal(replaced.activeProvider, 'deepseek')
  ...
  const staleClientResponse = await fetch(`${baseUrl}/explain`, {
    method: 'POST',
    headers: websiteHeaders,
    body: JSON.stringify({ term: '当前模型', provider: 'deepseek', model: 'deepseek-chat' }),
  })
  const staleClientResult = await staleClientResponse.json()
  assert.equal(staleClientResponse.status, 200)
  assert.equal(staleClientResult.provider, 'OpenAI')
  assert.equal(staleClientResult.providerId, 'openai')
  assert.equal(staleClientResult.model, 'gpt-5.2')
  assert.equal(completionModels.at(-1), 'gpt-5.2')
  ...
  assert.equal(completionModels.length, 2)
  ...
  assert.equal(overviewAfterDelete.activeProvider, 'deepseek')
  assert.equal(overviewAfterDelete.activeModel, 'deepseek-chat')
  const providerRequestsBeforeDeletedActivation = providerRequestCount
  const activationAfterDelete = await fetch(`${baseUrl}/ai/active`, {
    method: 'PUT',
    headers: websiteHeaders,
    body: JSON.stringify({ provider: 'openai', model: 'gpt-4o-mini' }),
  })
  assert.equal(activationAfterDelete.status, 503)
  assert.equal((await activationAfterDelete.json()).code, 'backend_not_configured')
  assert.equal(providerRequestCount, providerRequestsBeforeDeletedActivation)

  console.log('PASS 新增提供方必须填写 API Key，验证后原子写入隔离配置文件')
  console.log('PASS 未填写 Key 可读取官方预置目录，但动态模型发现必须使用当前凭据')
  console.log('PASS 五个提供方共享 OpenAI 兼容适配器并过滤非文本模型')
  console.log('PASS 第二个提供方保存后不抢占当前服务，用户可以再手动切换')
