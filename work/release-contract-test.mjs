...
assert.match(readme, /第 1 步：让 AI 安装网站和本机服务/)
assert.match(readme, /```text\n请帮我在这台电脑上安装并打开「加简大白话 · Plainify」/)
assert.match(readme, /Codex、Claude Code、Cursor Agent、Trae/)
assert.match(readme, /普通的豆包、ChatGPT 或 Claude 聊天窗口/)
assert.match(readme, /判断当前系统是 Windows 10\/11 还是 macOS/)
assert.match(readme, /不要让我手动下载、解压或双击文件/)
assert.match(readme, /最后打开 http:\/\/127\.0\.0\.1:5173\//)
assert.doesNotMatch(readme, /```powershell\n& \{/)
assert.doesNotMatch(readme, /plainify_installer="\$\(mktemp -t plainify-install\)"/)
