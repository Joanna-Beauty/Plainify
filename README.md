...
**使用 macOS？直接进入[四步快速开始](#macos-快速开始)。**
...
> [!IMPORTANT]
> 下面的一键安装适合使用 macOS 和 Chrome 或 Edge 的用户；Windows 和 Linux 用户可以按后文手动运行。你还需要任选一家受支持的模型服务商并准备 API Key。术语和 API Key 保存在自己的电脑上，但生成解释时，你主动提交的文字会发送给所选模型服务商。当前没有账号、云同步和自动备份。

## macOS 快速开始

从进入 GitHub 到完成第一次解释，一共四步。没有使用过 Git 或终端也可以按下面的顺序操作。

### 第 1 步：让 AI 帮你安装网站和本机服务
...
4. 在文件选择窗口按 `Command + Shift + G`，粘贴 `~/Applications/Plainify/extension`，按回车后选择这个文件夹。
