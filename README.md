...
**在网页里选中看不懂的术语或短句，把它变成大白话，并在以后遇到时自动高亮复习。**

加简大白话是一套在自己电脑上运行的阅读辅助工具。你可以先看一次解释，再决定是否把内容收进个人术语库，不需要注册账号。

**使用 Windows 或 macOS？直接进入[四步快速开始](#windows-和-macos-快速开始)。**

![在网页中选中文字、生成大白话解释并加入术语库](docs/assets/explain-selected-text.gif)

选中文字，点击 **用大白话解释**，就会看到通俗解释和生活化类比。觉得以后还会遇到时，再把它加入术语库。

> [!IMPORTANT]
> Windows 10/11 和 macOS 都支持一键安装，浏览器请使用 Chrome 或 Edge。你还需要任选一家受支持的模型服务商并准备 API Key。术语和 API Key 保存在自己的电脑上，但生成解释时，你主动提交的文字会发送给所选模型服务商。当前没有账号、云同步和自动备份。

## Windows 和 macOS 快速开始
...
1. 在加简大白话网站点击左侧 **设置**，再点击 **模型**。
2. 点击 **添加模型服务**，从 DeepSeek、OpenAI、阿里云百炼、Moonshot AI 或智谱 AI 中任选一个。
3. 保持 **官方 API**，点击 **获取 API Key**，按服务商页面提示创建 Key。
4. 把 Key 粘贴回加简大白话，点击 **联通测试并获取模型**。
5. 看到“联通成功”后选择一个模型，再点击 **保存并开始使用**。

API Key 可以理解为模型服务商发给你的“调用密码”，可能消耗免费额度或产生费用。完整配置说明和安全说明见[配置模型 API Key](#配置模型-api-key)。

### 第 3 步：把扩展加载到 Chrome 或 Edge
...
1. 打开任意一篇普通网页。浏览器设置页、新标签页和扩展商店等内部页面不能使用扩展。
2. 用鼠标选中一个不懂的词、句子或短段落，最多 120 个字符。
3. 点击选中文字旁边的 **用大白话解释**。
4. 只想临时查看时点击 **先不添加**；想以后复习时点击 **加入术语库**。

看到“大白话解释”和“生活化类比”，就表示安装、扩展和模型都已经连接成功。加入术语库后，这段内容会显示黄色高亮，以后在其他网页再次遇到时也能直接复习。

> [!TIP]
> 某一步没有出现预期结果时，直接查看[常见问题](#常见问题)。最常见的原因是扩展加载后没有刷新网页，或模型还没有通过联通测试。

## 安装后可以做什么

- **边读边解释**：在技术文档、英文文章、财经内容或办事指南中选中短内容，直接查看大白话解释。
- **把值得记住的内容收起来**：从网页、扩展弹窗或术语库手动收录，并按主题整理。
- **在下一次遇到时复习**：已收录的术语会自动显示黄色高亮，也可以在术语库中进行快速复习。

![加简大白话术语库首页](docs/assets/plainify-library.png)

## 完整使用说明

前面的快速开始足以完成第一次解释。下面保留完整安装、使用和排障说明，需要时再按目录查看。

## 目录

- [Windows 和 macOS 快速开始](#windows-和-macos-快速开始)
- [开始前需要准备什么](#开始前需要准备什么)
- [下载与版本](#下载与版本)
...
Windows 和 macOS 用户也可以在项目文件夹的终端中运行下面两条命令，重新安装并检查常驻服务：
...
- API Key 保存在项目根目录的 `.env.local` 中；macOS/Linux 使用 `0600` 文件权限，Windows 移除继承权限并只授权当前用户读写。
...
- 未公开修复的漏洞请按 [安全政策](SECURITY.md) 私密报告，不要把漏洞细节或真实凭据发到公开 Issue。
...
本项目采用 [MIT License](LICENSE) 开源。你可以在许可证约定下使用、修改和分发本项目。
